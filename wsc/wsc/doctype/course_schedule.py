...
def get_course_schedule_events(start, end, filters=None):
	...
	data = frappe.db.sql(
			"""select name, course, course_name, course_code,color,program,instructor,instructor_name,additional_trainer_1_name,additional_trainer_2_name,
	...
	for d in data:
		...
		res = {"course":str(d.course_name)+"\n"+str(d.school_house)+"\n"+str(d.room_name)+"\n"+str(d.instructor_name)+"\n"+(str(d.additional_trainer_1_name) if d.additional_trainer_1_name is not None else "")+"\n"+(str(d.additional_trainer_2_name) if d.additional_trainer_2_name is not None else "")}
		d.update(res)
		# +"\n"+to_time+"\n"+from_time
		# +d.school_house+"\n"
		result.append(d)
