...
class PlacementTool(Document):
    @frappe.whitelist()
    def declare_round(self, round_status, drive_name, round_name):
        if(round_status == 'Scheduling Of Round'):
            set_round_status = frappe.db.sql(""" UPDATE `tabRounds of Placement` SET round_status = 'Round Scheduled' WHERE parent = '%s' and round_name = '%s' """%(drive_name, round_name))
            pass
...
# class PlacementTool(Document):
#     @frappe.whitelist()
#     # creates records in selection round doctype
#     def schedule_round(self):
#         pass

# @frappe.whitelist()
# # gets the eligible students from the placement drive
# def get_student(drive_name):
#     pass

# @frappe.whitelist()
# # get date and time of the round
...
# # @frappe.whitelist()
# # # get rounds of placement
# # def get_rounds_of_placement(self, drive_name = None, round_status = None):
# #     all_round_names = frappe.db.sql()
# #     rounds_scheduled = frappe.db.sql()
# #     rounds_not_scheduled = frappe.db.sql()
# #     # for result declaration the last value of the rounds_scheduled will be taken
# # 	# for round scheduling the first value of rounds_not_scheduled will be taken
# #     if(self.round_status == 'Scheduling Of Round'):
# #         round_tobe_scheduled_idx = rounds_not_scheduled[0][0]
# #         round_tobe_scheduled_name = rounds_not_scheduled[0][1]
# #         if('1'):
...
# @frappe.whitelist()
# def get_rounds_of_placement(drive_name = None, round_status = None):
#     all_round_names = frappe.db.sql(""" SELECT idx, round_name FROM `tabRounds of Placement` WHERE parent = '%s' ORDER BY idx ASC """ %(drive_name))
#     rounds_not_scheduled = frappe.db.sql(""" SELECT idx, round_name FROM `tabRounds of Placement` WHERE parent = '%s' AND round_status = 'Not Scheduled' ORDER BY idx ASC"""%(drive_name))
#     rounds_scheduled = frappe.db.sql(""" SELECT idx, round_name FROM `tabRounds of Placement` WHERE parent = '%s' AND round_status = 'Scheduled' ORDER BY idx """%(drive_name))
#     round_results_declared = frappe.db.sql(""" SELECT idx, round_name FROM `tabRounds of Placement` WHERE parent = '%s' AND round_status = 'Result Declared' ORDER BY idx """%(drive_name))
#     if(round_status == 'Scheduling Of Round'):
#         print(rounds_not_scheduled)
#         return rounds_not_scheduled[0][1]
#     elif(round_status == 'Round Result Declaration'):
#         if(len(rounds_scheduled) != 0):
#             return 'round 9'
...
#     elif(round_status == 'Round Result Declaration'):
#         if(len(rounds_scheduled) == 0):
#             return 'No round has been scheduled'
#         elif(len(round_results_declared) == len(all_round_names)):
#             return 'All results have been declared'
#         elif(len(rounds_scheduled) != 0):
#             return 'round 9'
...
# # 	current_round_scheduling_status = frappe.db.sql(""" SELECT COUNT(*) FROM `tabSelection Round` parent = '%s' AND round_of_placement = '%s' AND round_status = 'scheduled'"""%(drive_name, current_round_name))
...
# # 			if(current_round_scheduling_status[0][0] == 0):
# # 				previous_round_result_status = frappe.db.sql(""" SELECT COUNT(*) FROM `tabSelection Round` parent = '%s' AND round_of_placement = '%s' AND round_status = 'result declared'"""%(drive_name, previous_round_name))
# # 				if(previous_round_result_status > 0):
# # 					return current_round_name
# # 	elif(self.round_status == 'Round Result Declaration'):
# # 		if(current_round_scheduling_status[0][0] > 1):
# # 			current_round_result_status = frappe.db.sql(""" SELECT COUNT(*) FROM `tabSelection Round` parent = '%s' AND round_of_placement = '%s' AND round_status = 'result declared'"""%(drive_name, current_round_name))
# # 			previous_round_result_status = frappe.db.sql(""" SELECT COUNT(*) FROM `tabSelection Round` parent = '%s' AND round_of_placement = '%s' AND round_status = 'result declared'"""%(drive_name, previous_round_name))
# # 			if(current_round_idx == 1):
