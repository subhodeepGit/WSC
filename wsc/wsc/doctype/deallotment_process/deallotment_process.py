...
def deallotment(self):
    ...
    De_allotment_info = frappe.db.sql("""SELECT `name`,`allotment_number`,`student`,`student_name`,`hostel`,`room_number`,
                                         `room_no`,`workflow_state`,`room_type`,`application_status` 
                                         FROM `tabDeallotment Process` WHERE `allotment_number`= "%s" """ % (Al_no))

    De_allotment_df = pd.DataFrame({
        'Deall_doc_no': [info[0] for info in De_allotment_info],
        'allotment_number': [info[1] for info in De_allotment_info],
        'student': [info[2] for info in De_allotment_info],
        'student_name': [info[3] for info in De_allotment_info],
        'hostel': [info[4] for info in De_allotment_info],
        'Room_id': [info[5] for info in De_allotment_info],
        'Room_no': [info[6] for info in De_allotment_info],
        'workflow_state': [info[7] for info in De_allotment_info],
        'room_type': [info[8] for info in De_allotment_info],
        'application_status': [info[9] for info in De_allotment_info]
    })
    ...
    if workflow_state == "Submit":
        ...
        if End_date is None:
            ...
            Al_data = frappe.db.sql("""SELECT `start_date`,`end_date`,`room_id` FROM `tabRoom Allotment` 
                                       WHERE `name`="%s" """ % (Al_no))
            ...
            if al_st_date <= End_date and al_end_date >= End_date:
                frappe.db.sql("""UPDATE `tabRoom Allotment` SET `end_date`="%s", `allotment_type`="De-Allotted" 
                                 WHERE `name`="%s" """ % (End_date, Al_no))
                ...
                frappe.db.set_value("Student Hostel Admission", status[0]['hostel_registration_no'], "allotment_status",
                                    "De-allotted")
            else:
