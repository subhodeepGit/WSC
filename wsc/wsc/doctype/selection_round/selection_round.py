...
def validate_cancel(self):
    ...
    result_record_status = frappe.db.sql(""" SELECT COUNT(*) FROM `tabSelection Round` WHERE docstatus = '1' AND drive_round_status == 'Round Result Declaration' AND placement_drive_name = '%s' AND round_of_placement = '%s' AND application_id = '%s'"""%(placement_drive_id, current_round_name, application_id))
    result_record_status = result_record_status[0][0]

    if(drive_round_count == 1):
        if(current_round_status == 'Scheduling Of Round'):
            if(result_record_status == 0):
                frappe.set_value('Placement Drive Application', application_id, 'status', 'Applied')
            elif(result_record_status > 0):
            ...
            if(current_round_status == 'Scheduling Of Round'):
                if(result_record_status == 0):
                    if(prev_result_record_status > 0):
                        prev_application_status = frappe.db.sql()
