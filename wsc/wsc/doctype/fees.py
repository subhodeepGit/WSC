...
class Fees(AccountsController):
    ...
    def calculate_total(self):
        ...
        if self.grand_total < 0:
            self.grand_total_in_words = "-(" + money_in_words(abs(self.grand_total)) + ")"
    ...
    def on_cancel(self):
        cancel_refund_amt(self)
        cancel_child_update(self)
        if self.is_return and self.return_against:
            return_against=frappe.get_doc("Fees",self.return_against)
...
def validate_amount(doc):
    if doc.is_return:
        for cmp in doc.components:
            if cmp.amount>=0:
                frappe.throw("Component <b>{0}</b> amount Must be <b>-ve</b> value and should not be <b>Zero</b>.".format(cmp.fees_category))
        if doc.grand_total>=0:
            frappe.throw("Grand Total Must be <b>-ve</b> value and should not be <b>Zero</b>.")
...
def get_fee_structures(doctype, txt, searchfield, start, page_len, filters):
    ...
    data=frappe.db.sql("""select name,program,student_category,academic_year from `tabFee Structure` where ({key} like %(txt)s or {scond}) 
                          and docstatus=1 and disable=0 and (program = '{program}' or academic_year = '{academic_year}' or  academic_term='{academic_term}' {student_category})
                         """.format(
                    **{
                        "key": searchfield,
                        "scond": searchfields,
...
def make_refund_fees(source_name, target_doc=None):
    def set_missing_values(source, target):
        ...
        for d in source.get("components"):
            target.append("components",{
                    "fees_category":d.fees_category,
                    "amount":-(d.grand_fee_amount-d.outstanding_fees)+d.custom_return_amount,
                    "description":d.description,
                    "income_account":d.income_account,
                    "receivable_account":d.receivable_account,
                    "custom_id_name":d.name,
                    "grand_fee_amount":-(d.grand_fee_amount-d.outstanding_fees)+d.custom_return_amount,
                    "outstanding_fees":-(d.grand_fee_amount-d.outstanding_fees)+d.custom_return_amount
                })
        target.grand_total=(-(source.grand_total-target.custom_return_grand_total-source.outstanding_amount))
        target.is_return=1
        target.return_against=source.name
...
def refund_amt(self):
    if self.return_against:
        amt = frappe.get_all("Fees",filters={"name": self.return_against,"docstatus":1}, fields=["name", "grand_total","custom_return_grand_total","outstanding_amount"])
        ret_amt = amt[0]['custom_return_grand_total']+(-self.grand_total)
        t_amt = amt[0]['grand_total'] - amt[0]['custom_return_grand_total']-amt[0]['outstanding_amount']
        if ret_amt <= amt[0]['grand_total']-amt[0]['outstanding_amount'] and self.grand_total < 0:
            frappe.db.set_value("Fees", amt[0]['name'], "custom_return_grand_total", ret_amt)
        else:
            frappe.throw(_("The Grand total must be negative, ranging from <b>{0}</b> and should not be <b>Zero</b>.").format(-t_amt))

def cancel_refund_amt(self):
    if self.return_against:
        amt = frappe.get_all("Fees",filters={"name": self.return_against,"docstatus":1}, fields=["name", "grand_total","custom_return_grand_total"])
        ret_amt = amt[0]['custom_return_grand_total']-(-self.grand_total)
        frappe.db.set_value("Fees", amt[0]['name'], "custom_return_grand_total", ret_amt)

def child_update(self):
    if self.return_against:
        target = frappe.get_doc('Fees', self.return_against)
        source = frappe.get_all("Fee Component", filters={"parent": self.name}, fields=["name", "custom_id_name", "amount","outstanding_fees","grand_fee_amount"])
        for s in source:
            for t in target.get("components"):
                if s["custom_id_name"] == t.name:
                    rt_amt = t.custom_return_amount - s["amount"]
                    if rt_amt <= (t.grand_fee_amount-t.outstanding_fees) and s["amount"]<0:
                        t.custom_return_amount = rt_amt
                    else:
                        frappe.throw(_("The Amount in the table must be negative, ranging from <b>{0}</b> and should not be <b>Zero</b>.").format(-((t.grand_fee_amount-t.outstanding_fees) - t.custom_return_amount)))
        target.save()

def cancel_child_update(self):
    if self.return_against:
        target = frappe.get_doc('Fees', self.return_against)
        source = frappe.get_all("Fee Component", filters={"parent": self.name}, fields=["name", "custom_id_name", "amount","outstanding_fees","grand_fee_amount"])
        for s in source:
