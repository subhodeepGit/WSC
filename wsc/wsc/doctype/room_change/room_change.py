...
def ra_query(doctype, txt, searchfield, start, page_len, filters):
    ...
    if frappe.session.user != "Administrator":
        if "Employee" in frappe.get_roles(frappe.session.user):
            ...
            if emp_id:
                Emp_al = frappe.db.sql("""
                    SELECT `hostel_masters` FROM `tabEmployee Hostel Allotment`
                    WHERE employees="%s" AND (`start_date` <= NOW() AND `end_date` >= NOW())
                """ % (emp_id[0]['name']))
                if Emp_al:
                    if len(Emp_al) == 1:
                        info = """AND hostel_id="%s" """ % (Emp_al[0][0])
                    else:
                        hostel = tuple(item[0] for sublist in Emp_al for item in sublist)
                        info = """AND hostel_id IN %s""" % str(hostel)
                else:
                    frappe.throw(_("<b>{0}</b> is not allotted to the <b>Employee Hostel Allotment.</b>").format(frappe.session.user))
############################### Search Field Code######################
    searchfields = frappe.get_meta(doctype).get_search_fields()
    searchfields = " OR ".join(field + " LIKE %(txt)s" for field in searchfields)

    data = frappe.db.sql("""
        SELECT `name`, `student`, `student_name`, `hostel_id`
        FROM `tabRoom Allotment`
        WHERE ({key} LIKE %(txt)s OR {scond})
            AND (`start_date` <= NOW() AND `end_date` >= NOW())
            AND (`allotment_type` NOT IN ("Hostel suspension", "Suspension", "Debar", "University Suspension",
                                          "School Suspension", "Death", "De-Allotted"))
            AND `docstatus` = 1 {info}
    """.format(
        **{
            "key": searchfield,
            "scond": searchfields,
