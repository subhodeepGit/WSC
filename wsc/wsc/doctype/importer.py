...
class ImportFile:
	...
	def read_content(self, content, extension):
		...
		# 	child_list = [str(i)[:-1] if isinstance(i, str) and i.endswith("*") else str(i) for i in list]
		...
		parent_list = []
		found_first_non_none_list = False
		...
		for list in data:
			...
			if not found_first_non_none_list and any(list):
				found_first_non_none_list = True
				for i in list:
					if "*" in str(i):
						child_list.append(str(i[:-1]))
					else:
						child_list.append(str(i))
