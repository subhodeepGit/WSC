...
class Exporter:
	...
	def get_csv_array_for_export(self):
		...
		# Create a mandatory fields dictionary where label is key and value is in binary
		reqd_list = [d['reqd'] for d in self.fields]
		reqd_dict = dict(zip(csv_array[0], reqd_list))

		# Create a list of modified label names with * marks for Mandatory fields
		label_list = [label + '*' if value == 1 else label for label, value in reqd_dict.items()]
		csv_array[0] = label_list
