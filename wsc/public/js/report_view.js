...
// #############################################################################################
					...
					d.$body.prepend(`
					<div class="mb-3">
					<button type="button" class="btn btn-default btn-check-all" style="margin-right: 5px;">${__("Check All")}</button>
					<button type="button" class="btn btn-default btn-uncheck-all" style="margin-right: 5px;">${__("Uncheck All")}</button>
					</div>
				`);

				// Bind click events for the buttons
				d.$body.on("click", ".btn-check-all", () => {
					d.$body.find(":checkbox:not(:checked)").prop("checked", true).trigger("change");
				});

				d.$body.on("click", ".btn-uncheck-all", () => {
					d.$body.find(":checkbox:checked").prop("checked", false).trigger("change");
				});
// ########################################################################################################
