...
def await_transaction_update_status():    
    ...
    awaited_status_transactions_1=frappe.get_all("OnlinePayment",[["transaction_status" ,"IN",("Awaited","Failure","Initiated","Success","Rejected","Aborted","Unsuccessful","Shipped")]])
    awaited_status_transactions_0=frappe.get_all("OnlinePayment",[["docstatus" ,"=",0]])
    # logging.info("t0 awaited_status_transactions_0: %s", awaited_status_transactions_0) 
        ...
        try:
            ...
            if doc.docstatus==0:
                ...
                if "order_no" in data_dict["Order_Status_Result"].keys():
                    ...
                    logging.info("t0 type order_no: %s", type(order_no))
                    order_status=data_dict['Order_Status_Result']['order_status']   
                    logging.info("t0 type order_status: %s", type(order_status))
                    payer_name=data_dict['Order_Status_Result']['order_bill_name'] 
                    logging.info("t0 type payer_name: %s", type(payer_name))
